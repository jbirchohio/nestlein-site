...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderCards(data) {
    ...
    const cards = data.map(loc => {
      ...
      const hours = loc.hours_of_operation || 'Unknown hours';
            ...
            <div class="absolute inset-0 backface-hidden rounded-xl overflow-hidden">
              \${loc.logo_url ? \`
                <div class="relative w-full h-40">
                  <img src="\${loc.logo_url}" alt="\${loc.name}" class="w-full h-full object-cover rounded-t-xl" />
              ...
              \` : ""}
              <div class="p-4">
                <h3 class="text-lg font-bold text-orange-800 text-center">\${loc.name}</h3>
              </div>
            </div>
            <div class="absolute inset-0 backface-hidden rotate-y-180 rounded-xl p-4 bg-orange-50 flex flex-col justify-center space-y-2 text-sm text-gray-800 overflow-y-auto">
