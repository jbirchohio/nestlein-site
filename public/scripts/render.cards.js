...
function renderCards(data) {
  const cards = data.map(loc => {
    ...
    return `
      <a href="/locations/${loc.slug}" class="flip-card relative w-full max-w-sm h-[360px] mx-auto group block no-underline">
        <div class="flip-inner w-full h-full transition-transform duration-500 [transform-style:preserve-3d] group-hover:rotate-y-180">
          ...
          <div class="card-front absolute inset-0 backface-hidden rounded-xl border border-orange-100 bg-white shadow overflow-hidden z-10">
            <div class="relative">
              ...
              <div class="absolute top-2 left-2 bg-white/80 px-2 py-0.5 rounded-full text-xs font-bold text-orange-800 shadow">${loc.name}</div>
            </div>
            <div class="p-3 relative h-[calc(100%-160px)]">
          ...
          <div class="card-back absolute inset-0 backface-hidden rotate-y-180 rounded-xl p-4 bg-orange-50 flex flex-col justify-center space-y-2 text-sm text-gray-800 z-0">
            <p class="text-gray-600 font-medium">${loc.address || ''}</p>
