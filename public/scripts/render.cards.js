...
function renderCards(data) {
  ...
  // Add flip/tap behavior for mobile and slug navigation
  document.querySelectorAll('.flip-card').forEach(card => {
    let tappedOnce = false;
    card.addEventListener('click', (e) => {
      const flip = card.querySelector('.flip-inner');
      if (window.innerWidth < 768) {
        if (!flip.classList.contains('rotate-y-180')) {
          e.preventDefault();
          flip.classList.add('rotate-y-180');
          tappedOnce = true;
        } else if (tappedOnce) {
