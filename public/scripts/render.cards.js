...
function renderCards(data) {
  const cards = data.map(loc => {
    ...
    return `
      <a href="/locations/${loc.slug}" class="block">
        <div class="flip-card relative w-full max-w-sm mx-auto group">
          <div class="flip-inner w-full transition-transform duration-500 [transform-style:preserve-3d] group-hover:rotate-y-180">
            ...
            <div class="card-front absolute inset-0 backface-hidden rounded-xl border border-orange-100 bg-white shadow overflow-hidden z-10">
              ...
              <div class="absolute top-2 left-2 right-2 flex justify-between items-center px-3">
                <h3 class="text-lg md:text-xl font-bold text-orange-800 truncate">${loc.name}</h3>
                <div class="bg-white/80 px-3 py-0.5 rounded-full text-xs font-semibold border border-orange-200 shadow">
            ...
            <!-- Back -->
            <div class="card-back absolute inset-0 backface-hidden rotate-y-180 rounded-xl bg-orange-50 p-4 flex flex-col justify-center text-sm text-gray-800 z-0">
              ...
              <div class="flex flex-wrap gap-1 mt-2">
                ${(loc.tags || []).map(tag => {
                  const baseColor = tag.toLowerCase().includes('coffee') ? 'bg-yellow-100 text-yellow-700'
                                   : tag.toLowerCase().includes('group') ? 'bg-green-100 text-green-700'
                                   : tag.toLowerCase().includes('quiet') ? 'bg-blue-100 text-blue-700'
